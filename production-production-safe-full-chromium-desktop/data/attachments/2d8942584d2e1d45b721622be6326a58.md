...
Locator: getByTestId('product-category-options').or(getByRole('complementary').filter({ hasText: /\uc0ac\uc774\uc988|\uc218\ub7c9|Size|Quantity/i })).first().getByRole('button', { name: '다음 단계' })
Expected: enabled
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByTestId('product-category-options').or(getByRole('complementary').filter({ hasText: /\uc0ac\uc774\uc988|\uc218\ub7c9|Size|Quantity/i })).first().getByRole('button', { name: '다음 단계' })
    3 × locator resolved to <button disabled type="button" data-size="md" data-v-0a80e1f8="" data-tone="default" data-variant="filled" data-selected="false" data-no-hover="false" data-icon-only="false" class="ui-button next-step-btn">…</button>
